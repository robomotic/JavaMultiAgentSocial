...
public /*strictfp*/ class AgentsForage extends SimState
    ...
    public static final int INITIAL_AGENTS = 10;
    ...
    public static final int FOOD_POINTS = 6;
        ...
        if(flag ^ isLearning)
        {
            if(society!=null && society.numObjs>0)
            {
                ...
                {                
                    ...
                    //update the entropy base 2 of the data collected since now
                    pointer.entropyReflexAgent.resetEntropy();
                    pointer.entropyReflexFood.resetEntropy();
                    pointer.entropyReflexAgentFood.resetEntropy();
                }
        ...
        Steppable updateAgents = new Steppable()
            ...
            public void step(SimState state)
                ...
                for(int k=0;k<society.numObjs;k++)
                {
                    ...
                    positiveTrust+=agent_obj.getPositiveTrust();
                    negativeTrust+=agent_obj.getNegativeTrust();
                    ...
                    //compute the entropy at runtime: VERY HEAVY better to analyse offline
                    if(ENTROPY_RUNTIME)
                    {
                    agent_obj.entropyReflexAgent.computeEntropy(2);
                    agent_obj.entropyReflexFood.computeEntropy(2);
                    agent_obj.entropyReflexAgentFood.computeEntropy(2);
                    agent_obj.entropyOutput.computeEntropy(2);
                    }
                    //distal and reflex signal for agent avoidance
                    record_avoid[3*k+1]=Integer.toString(agent_obj.entropyReflexAgent.getLastBin());
                    record_avoid[3*k+2]=Integer.toString(agent_obj.entropyDistalAgent.getLastBin());
                    record_avoid[3*k+3]=Double.toString(agent_obj.ico.getWeight(LowPassFilter.AGENT_SIGNAL));
                    
                    record_agent_attraction[3*k+1]=Integer.toString(agent_obj.entropyReflexAgentFood.getLastBin());
                    record_agent_attraction[3*k+2]=Integer.toString(agent_obj. entropyDistalAgentFood.getLastBin());
                    record_agent_attraction[3*k+3]=Double.toString(agent_obj.ico.getWeight(LowPassFilter.AGENTFOOD_SIGNAL));
                    
                   record_food_attraction[3*k+1]=Integer.toString(agent_obj.entropyReflexFood.getLastBin());
                   record_food_attraction[3*k+2]=Integer.toString(agent_obj.entropyDistalFood.getLastBin());
                   record_food_attraction[3*k+3]=Double.toString(agent_obj.ico.getWeight(LowPassFilter.FOOD_SIGNAL)); 
                    try {
